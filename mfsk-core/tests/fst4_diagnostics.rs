...
//! question during a specific investigation: #146's AWGN gap, #198's
//! f32-hardcoded `decode_soft`, #306's embedded feasibility, #308's
//! `i0` jitter retry, #310's rung-major scheduling and soft
//! Costas-margin proposal, #312's sniper candidate cap. Most of those
//! are closed. Several of these tests are the *refutation* that closed
//! them — `soft_costas_margin_separation`, `_escalation_priority` and
//! `_conditional_value` are the three steps that talked #310's proposal
//! down, and the docs on each carry the numbers that did it.
//!
//! They live here rather than in `fst4_sweep.rs` because that file is
//! the tier-C gate and these are not. Mixing them put 5 800 lines of
//! closed investigation in front of the ~500 lines anyone actually runs
//! before a release. Nothing is deleted: a design decision whose
//! evidence is no longer executable is a decision nobody can re-check.
//!
//! Run one by name:
//!
//! ```sh
//! cargo test -p mfsk-core --features full,internal-testing --release \
//!     --test fst4_diagnostics <name> -- --ignored --nocapture
//! ```
//!
