...
///
/// This predicate is always available. Whether the message validators below
/// *accept* a CB identifier is the opt-in `cb-callsigns` feature; without it
/// they stay strict ITU-amateur, as before.
...
/// Check whether a string is a valid FT8 callsign (standard, non-standard,
/// or CB).
///
/// Accepts callsigns per ITU Radio Regulations and FT8 encoding:
///
/// 1. **Standard** (pack28 format): handled by [`is_standard_callsign`].
/// 2. **CB** (only with the `cb-callsigns` feature): the 11 m identifiers
///    [`is_cb_callsign`] accepts — they have no amateur structure to validate
///    against, so they are admitted as a callsign outright and travel on-air
///    through the free-text and hash-carrying layouts, as they do in WSJT-CB.
/// 3. **Base callsign** without modifiers: e.g. `3DA0WPX` (7-char, Type 4).
/// 4. **Compound callsign** with `/`:
...
/// - **CB identifiers** ([`is_cb_callsign`], only with the `cb-callsigns`
///   feature) pass without the allowlist — they have no ITU amateur prefix to
///   check, and their numeric-plus-numeric structure is distinctive enough not
///   to be a faulty-CRC landing spot.
pub fn is_plausible_callsign(call: &str) -> bool {
    // CB identifiers have their own structure — a numeric prefix and a
    // numeric unit — so they pass without the ITU amateur prefix allowlist.
    #[cfg(feature = "cb-callsigns")]
    if is_cb_callsign(call) {
        return true;
    }
...
mod tests {
    ...
    #[cfg(feature = "cb-callsigns")]
    #[test]
    fn cb_callsigns_are_valid_and_plausible() {
        for &(call, accepted) in WSJT_CB_TABLE.iter().filter(|(_, a)| *a) {
            assert!(
                is_valid_callsign(call),
                "should accept {call} as a callsign"
            );
            assert!(
                is_plausible_callsign(call),
                "should pass plausible for {call}"
            );
        }
    }
    ...
    fn cb_callsigns_comply_with_pack28_when_standard() {
        ...
        // like `1TT1` fall back to free-text / hashed, as WSJT-CB. `1A1` is
        // the dual-shape overlap: it is a CB call *and* a legal 3-char
    ...
    #[cfg(feature = "cb-callsigns")]
    #[test]
    fn cb_messages_pass_the_plausibility_gate() {
        for m in [
            "26AT715 RR73",
            "CQ 26AT715",
            "1A1 26AT715 73",
            ...
            assert!(is_plausible_message(m), "plausible_message({m}) must pass");
        }
    }

    /// Without the opt-in, the validators stay exactly as strict as before:
    /// a CB-only identifier is not a callsign and not plausible.
    #[cfg(not(feature = "cb-callsigns"))]
    #[test]
    fn cb_callsigns_rejected_without_the_feature() {
        for call in ["26AT715", "1TT1", "1TT1000", "999ZZ/ZZ"] {
            assert!(
                !is_valid_callsign(call),
                "{call} must be rejected without cb-callsigns"
            );
            assert!(
                !is_plausible_callsign(call),
                "{call} must not be plausible without cb-callsigns"
            );
        }
        // `1A1` is simultaneously a legal 3-char standard call, so it stays
        // accepted either way — the feature only widens, never narrows.
        assert!(is_valid_callsign("1A1"));
    }
