...
/// Repeatedly apply [`subtract_tones_lpf`] to the same candidate until the
/// marginal improvement plateaus (or starts to reverse), instead of a
/// single shot.
///
/// # Why
///
/// A single `subtract_tones_lpf` call only reaches the suppression depth
/// its one-shot LS/LPF amplitude estimate happens to hit — on a real
/// (non-synthetic) signal this is often shallow (e.g. ~6.6 dB measured on
/// a real fading FT8 signal, vs > 100 dB achievable on a clean synthetic
/// self-test). WSJT-X's `subtractft8.f90` doesn't do anything
/// algorithmically different — but its multi-pass/iaptype decode loop
/// happens to call `subtractft8` on the *same* strong candidate several
/// times as it's re-encountered, and each call estimates and removes
/// whatever's *currently* left, converging like a fixed-point iteration
/// (empirically ~17.65 dB cumulative over 6 calls on the same signal,
/// diminishing each time: 3.9, 0.5, 3.0, 3.3, 3.4, 3.5 dB steps).
///
/// # Why not just always iterate `max_iters` times
///
/// Tried that first (issue #177) — a fixed iteration count regresses
/// other, already-well-estimated candidates: continuing to subtract past
/// the point where there's nothing real left to remove doesn't leave the
/// residual unchanged, it *overshoots* — the per-iteration LS/LPF
/// re-estimate on an already-near-zero residual starts fitting to noise
/// and can subtract signal energy that was never really there, and the
/// per-iteration suppression measurement actually goes *negative*
/// (residual gets *louder*, i.e. ringing) once that happens. That
/// negative-step signature is directly observable and is what this
/// function stops on, rather than guessing a fixed count.
///
/// # Stopping rule
///
/// Measures the LS amplitude magnitude (same metric [`refine_freq`]
/// uses internally — cheap, no FFT) before and after each
/// application. Stops as soon as an iteration's dB improvement drops
/// below `min_step_db`, or is negative (overshoot), or `max_iters` is
/// reached. Returns the number of `subtract_tones_lpf` calls actually
/// applied.
///
/// Recommend `max_iters = 6`, `min_step_db = 1.0` — calibrated against
/// `qso3_busy.wav`'s 18-signal golden set (see
/// `ft8_qso3_sync_cv_iteration_correlation.rs` and
/// `ft8_qso3_subtract_fix_check.rs` in the `mfsk-core` integration
/// tests): every genuinely-fading signal in that set (e.g. W1FC
/// F5BZB, `sync_cv=0.67`) still shows > 1 dB improvement through at
/// least 4 iterations, while every signal a fixed 3x pass regressed
/// (e.g. K1JT HA5WA 73) drops under 1 dB — or goes negative — by
/// iteration 2-3.
///
/// Note: `sync_cv` (the existing QSB/fading flag used elsewhere for
/// partial-gain compensation) does *not* predict this — measured
/// correlation between `sync_cv` and either first-iteration dB or
/// total achievable suppression across the qso3_busy.wav golden set is
/// ~0.13 (near zero). Convergence behaviour has to be measured
/// per-candidate, not inferred from `sync_cv`.
#[allow(clippy::too_many_arguments)]
pub fn subtract_tones_lpf_converge(
    audio: &mut [i16],
    tones: &[u8],
    freq_hz: f32,
    dt_sec: f32,
    ...
    let mut prev_amp = ls_amp_mag(audio, tones, freq_hz, dt_sec, cfg);
    let mut applied = 0u32;
    for _ in 0..max_iters {
        subtract_tones_lpf(audio, tones, freq_hz, dt_sec, cfg, lpf_half, endcorrection);
        applied += 1;
        let amp = ls_amp_mag(audio, tones, freq_hz, dt_sec, cfg);
        // amp is a magnitude (linear), suppression in dB is a *power*
        // ratio, so 20*log10(amp ratio) == 10*log10(power ratio).
        let step_db = if amp > f32::EPSILON {
            20.0 * (prev_amp / amp).log10()
        } else {
            f32::INFINITY
        };
        prev_amp = amp;
        let converged = matches!(
            step_db.partial_cmp(&min_step_db),
            None | Some(core::cmp::Ordering::Less)
        );
        if converged {
            // None (NaN) or Less covers NaN/negative (overshoot) — stop
            // immediately, don't keep pushing a candidate that's
            // started ringing.
            break;
        }
    }
