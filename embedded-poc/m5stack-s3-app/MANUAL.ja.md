...
M5StickS3 GPIO map は `src/board.rs` 参照 (KEY1 = GPIO 11、KEY2 =
...
完全な状態機械は `mfsk-app-shared/src/qso.rs` 参照。
...
| デコーダ動いてるが 0 decode | 音声 level 過小 / 過大、または IC-705 の band 違い | `audio capture+tx tick: NNN B/s rx` ログ行を見る — 健全な 48 kHz stereo I2S RX なら 192–196 kB/s 持続。無音なら mic gain (`audio.rs` `mic_gain_db`) や無線機出力 level 確認 |
...
- `embedded-poc/CLAUDE.md` — 共通組込ツールチェインメモ (cross-board)
- `embedded-poc/m5stack-s3-app/CLAUDE.md` — この crate での AI agent 向けボード固有メモ
- `docs/EMBEDDED.ja.md` — 全 embedded crate (S3、Core2、計画中 CoreS3) の最上位アーキテクチャ
- `docs/ROADMAP.md` — リリースマイルストーン + phase 計画
- `mfsk-core/` — 実体のデコーダライブラリ (host + embedded shared、crates.io 公開)
- `mfsk-app-shared/` — ボード非依存アプリロジック (QSO FSM、UI primitive、WiFi、log fanout)
