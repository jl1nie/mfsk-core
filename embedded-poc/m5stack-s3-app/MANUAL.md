...
See `src/board.rs` for the M5StickS3 GPIO map (KEY1 = GPIO 11, KEY2 =
...
The full state machine lives in `mfsk-app-shared/src/qso.rs`.
...
| Decoder runs but 0 decodes | Audio level too low / too high; or wrong band on IC-705 | Watch `audio capture+tx tick: NNN B/s rx` log lines — should be 192–196 kB/s sustained for healthy 48 kHz stereo I2S RX. If silent, check mic gain (`audio.rs` `mic_gain_db`) or radio output level. |
...
- `embedded-poc/CLAUDE.md` — shared embedded toolchain notes (cross-board).
- `embedded-poc/m5stack-s3-app/CLAUDE.md` — board-specific notes for AI agents working on this crate.
- `docs/EMBEDDED.md` — top-level architecture across all embedded crates (S3, Core2, planned CoreS3).
- `docs/ROADMAP.md` — release milestones and phase plan.
- `mfsk-core/` — the actual decoder library (host + embedded shared, published on crates.io).
- `mfsk-app-shared/` — board-agnostic app logic (QSO FSM, UI primitives, WiFi, log fanout).
