...
| `.tx_freq(hz)` | `f32` | unset | FT8 | the transmit frequency (`nftx`): FT8 also tries the both-callsigns hypotheses within 50 Hz of it. Ignored by the other modes (`ft4_decode.f90` has no `nftx`); the C ABI, which refuses what a mode lacks, publishes it as `MFSK_CAP_TX_FREQ` on FT8 alone |
