...
**状態 (2026-08-29): ビルドが通り、実機で正しくデコードし、スロット
予算を 8.8 倍超過している。** ボトルネックは単一の関数。
...
もう1つ要るが、ベンチは候補リストを焼くことで回避している。この段は
ホストのスロット全体で 0.3 ms。)
...
ではなく DDC フロントエンドが丸ごと消す種類の段——FST4 に既に
テンプレートがある `mfsk_core::ft4::ddc`
（`docs/notes/FST4_DDC_DESIGN.md`）であり、このベンチがまだ依存して
いるホスト焼きの広帯域 FFT も同時に不要になる。
