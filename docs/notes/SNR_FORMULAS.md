...
| JT65 | `jt65_decode.f90:251,254-255` (`s2db = 10·log10(sync2) - 35`, then clamp to `[-30,-1]`) | `jt65::rx::demodulate_aligned` (`Jt65Demod::snr_db`) — **own** estimator, not the generic heuristic | **Audited, formula deliberately not ported** — the existing estimator already lands within ~0.7 dB of real `jt9`; only WSJT-X's display clamp was adopted (issue #255) |
