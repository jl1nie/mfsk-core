...
## 17. The on-device bring-up log, moved here from EMBEDDED.md (2026-09-16)

The eighteen numbered attempts below were `EMBEDDED.md`'s
`## FST4 on embedded` section — 782 lines, 39% of that document, with
the attempts written as bold paragraphs rather than headings so they
appeared in no outline and no heading search. They are a measurement
journal, which is what this file is for; the reference manual now
carries the resulting status and a pointer here.

Moved verbatim. Where an attempt says "the same figure the Seventeenth
attempt closed with", the Seventeenth attempt is below, not elsewhere.
