...
streaming-DDC proposal). **A plan, not a result** — nothing here had
been run on hardware when this was written.

> It has since been run. The measurement and what it decided are in
> [`WSPR_EMBEDDED_MEASUREMENT_RESULTS.md`](WSPR_EMBEDDED_MEASUREMENT_RESULTS.md)
> (measured 2026-08-13), which is the document to read for the answer;
> this one is the criteria it was judged against, kept because a plan
> written after its results is worth nothing. Where the two disagree,
