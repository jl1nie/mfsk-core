...
- Equivalence at the decoder boundary: feed `ladder_cases.txt`'s `ZS`/`ZH`
  (correlations generated by `jtty_ladder_oracle`, not extracted from
  signals) to the Rust ladder and require the **same top-4 words in the same
  order, with metrics equal to f64 tolerance** (f64 accumulation, as
  upstream), and the same accepted rung. This isolates the trellis from DSP
  differences. Correlations extracted from the real recording's frames would
  add a check on the correlation stage; not part of P0.
